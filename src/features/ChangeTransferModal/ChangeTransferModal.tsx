...
export const ChangeTransferModal: FC<ChangeTransferModalProps> = ({ isOpen, onClose, onClick, cars, current, type }) => {
    ...
    const ConfirmMark = <div className={s.currentIcon}><ConfirmIcon /></div>;
    const CarMark = <div className={s.headerIcon}><CarIcon /></div>;
    ...
    return (
        <Modal title='Changing transfer option' isOpen={isOpen} onClose={onClose} >
            <div className={s.container}>
                ...
                <div className={cn(s.current, { [s.selected]: transferType === TransferType.PICKUP })} onClick={() => setTransferType(TransferType.PICKUP)}>
                    ...
                    <div className={s.currentDescription}>Sed mauris tincidunt justo malesuada pharetra, viverra in arcu ultrices. Donec aliquam nulla massa viverra aliquam diam ac leo.</div>
                </div>
                <div className={cn(s.cars, { [s.selected]: transferType === TransferType.RENTAL })}>
                                ...
                                return <TabPanel className={s.tabPanel}>
                                    <div className={s.carsList}>
                                        {getCarsByGroup(cars, group).map((car) => {
                                            const isCarSelected = selectedCar === car.id && transferType === TransferType.RENTAL
                                            return <CarCard isSelected={isCarSelected} onClick={(id) => selectCar(id)} {...car} />
                                        })}
                                    </div>
                                </TabPanel>
                            })}
