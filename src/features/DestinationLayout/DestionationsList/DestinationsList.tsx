...
export const DestionationsList: FC<DestionationsListProps> = ({ destination, destinations }) => {
  ...
  return (
    <div className={s.list}>{destinations.map((place) => {
      return <div onClick={() => router.push(`/destinations/${place.id}`)} className={cn(s.item, { [s.active]: place.id === destination })}>
      ...
      {isRootDestinations(destinations) && <div onClick={() => router.push('/destinations')} className={cn(s.item, { [s.active]: destination === 0 })}>
